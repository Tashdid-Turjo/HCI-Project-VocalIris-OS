...
import pyautogui
import time         # Added for timing the override
...
while cam.isOpened():
    ...
    # If the mouse moved and it wasn't our code that moved it
    ...
    if detection_result and detection_result.face_landmarks:
        # Only move mouse if user hasn't moved it manually in the last 1 second
        if time.time() - last_manual_move_time > 1.0:
            ...
            # Update our reference of where the code last moved the mouse
            last_mouse_x, last_mouse_y = mouse_x, mouse_y
        
        else:
            # If in manual mode, update reference to current position
