...
class HomePage(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # Configure layout weights (Row 0 = Wizard Banner, Row 1 = Left/Right Split Cards)
        ...
        self.calib_btn = ctk.CTkButton(
            self.calibration_card, 
            text="Launch Calibration Wizard", 
            fg_color="#2b82c9", hover_color="#1e5d91"
        )
        ...
        self.voice_title = ctk.CTkLabel(
            self.voice_card, text="🗣️ Voice Control Engine", 
            font=ctk.CTkFont(size=16, weight="bold")
        )
        self.voice_title.pack(anchor="w", padx=15, pady=15)
        
        # Service Master Toggle Switch
        self.voice_switch = ctk.CTkSwitch(self.voice_card, text="Start Voice Service Listener")
        self.voice_switch.pack(anchor="w", padx=20, pady=5)
        
        # Command Registry Cheat-Sheet Readout Box
        ...
        # Insert our structured command rules cheat-sheet text
        cheat_sheet = (
        ...
        self.cmd_box.configure(state="disabled") # Set to read-only descriptive panel
        ...
        self.eye_title = ctk.CTkLabel(
            self.eye_card, text="👁️ Eye Tracking Engine", 
            font=ctk.CTkFont(size=16, weight="bold")
        )
        self.eye_title.pack(anchor="w", padx=15, pady=15)
        
        # Master Tracker Active Toggle Switch
        self.eye_switch = ctk.CTkSwitch(self.eye_card, text="Start Eye Tracking Engine")
        self.eye_switch.pack(anchor="w", padx=20, pady=5)
        
        # Model Fallback Selector Dropdown Form
        ...
        # Visual Dummy Box Placeholder simulating where our local camera feed matrix will render later
